...
* **Protocol Selection:** During initial device setup, the firmware/companion-app must prompt the user to explicitly select their preferred networking protocol (ZigBee or Wi-Fi) to dictate how the ESP32-C6 operates.
* **Ecosystem Integration (Future Expansion):** The software architecture should be built with future integrations in mind, specifically planning for eventual support of Amazon Alexa, Google Home, and Apple Home (potentially utilizing the Matter protocol).
  * **Option: Matter "Late Provisioning":** If the Matter protocol is selected for mass production, the firmware must utilize a "Late Provisioning" system. This allows the factory (e.g., JLCPCB) to flash 100% identical firmware to every PCB. Upon the first boot in the user's home, the ESP32-C6 will self-generate its unique Matter credentials (saved to its NVS partition) and broadcast a temporary Setup Wi-Fi network displaying its unique pairing QR Code on a local captive webpage.
* **Power-Loss State Memory:** Implementing a state-recovery feature similar to Philips Hue. The firmware must save the active brightness and color settings to Non-Volatile Storage (NVS). Upon restoring power after an outage or physical unplug, the lamp must default to its last known state automatically.
